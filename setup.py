...
FILE_MAP = {
    ...
    # tools/ package
    "code_tools.py":        "tools/code_tools.py",

    # Bundled skills
    "coding.md":            "skills/coding.md",
    "testing.md":           "skills/testing.md",
    "documentation.md":     "skills/documentation.md",
    "devops.md":            "skills/devops.md",

    # Example rules (copied to .shuki.example/, not installed as active rules)
    "no_print_statements.md":   ".shuki.example/no_print_statements.md",
    "always_type_hints.md":     ".shuki.example/always_type_hints.md",
    "prefer_pathlib.md":        ".shuki.example/prefer_pathlib.md",
}

# Files that need an empty __init__.py created alongside them
PACKAGE_DIRS = ["agent", "tools"]

# Directories to create under %LOCALAPPDATA%\.shuki\ (user config dirs)
SHUKI_SUBDIRS = ["rules", "skills"]
...
def _move(src: Path, dst: Path, dry_run: bool):
    action = "copy " if dst.exists() else "move "
    _print(f"  {action} {src.name:35s} → {dst}", dry_run)
    if not dry_run:
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(src, dst)   # copy so re-running is safe
...
def setup(source_dir: Path, install_dir: Path, dry_run: bool):
    ...
    # 5. Create %LOCALAPPDATA%\.shuki\{rules,skills}
    shuki_root = _localappdata() / ".shuki"
    print()
    print(f"User config dirs ({shuki_root}):")
...
3. (Optional) Add your coding rules:
       ...
       copy "{install_dir / '.shuki.example' / '*'}" "{shuki / 'rules'}\\"
